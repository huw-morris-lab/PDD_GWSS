...
In this new colocalization analysis, we aim to identify whether our variants within LRP1B can be interpreted as cis-acting splicing quantitative trait loci (sQTL), what could lead us to think that a specific LRP1B mRNA isoform is linked to the progression to dementia phenotype.
...
loc_1 = "/data/kronos/kronos/acarrasco/R_libs/"
loc_2 = "/mnt/rreal/RDS/RDS/acarrasco/R_libs"
# This lib path does not work well. There is something wrong here
#Maybe arrow lib? installation messed up? This is the last thing I madofied on the specific environment
library(tidyverse)
library(data.table)
library(colochelpR, lib.loc=loc_2)
#library(MafDb.1Kgenomes.phase3.hs37d5)
library(SNPlocs.Hsapiens.dbSNP144.GRCh38, lib.loc=loc_1)
#library(biomaRt)
library(DT, lib.loc=loc_2)
library(coloc, lib.loc= loc_2)
library(arrow, lib.loc = loc_2)
```
...
Of note, since we are focused on LRP1B, I will do the analysis over chr2 only. 
...
For now, let's just going to explore chr2 gtex_v8_sQTL_Cortex EUR ancestry whole data.
...
survDementia = fread("/mnt/rreal/RDS/RDS/acarrasco/COLLABS/FOR_RAQUEL/DATA/Raquel_LRP1B_progToDementia/harmonised_progtoDemGWAS_V3.txt")

# Parquel format gtex cortex chrom2
gtex_cortex_chr2 = arrow::read_parquet("/mnt/rreal/RDS/RDS/DATA/GTEX_V8/gtexv8_download/GTEx_Analysis_v8_QTLs/GTEx_Analysis_v8_EUR_sQTL_all_associations/Brain_Cortex.v8.EUR.sqtl_allpairs.chr2.parquet")
```


# GTEX / dementia GWAS wrangling

To start with, we are going to use data from cortex as a start point.
Again, since all the GTEX_V8 is unprocessed, and the work with GTEX DATA is not automated,I will just load data from chr2 (LRP1B).
```{r eval = FALSE}
...
# We also make sure there is no duplicates at the intronic level
...
```{r echo=FALSE}
gtex_cortex_chr2_processed_nodups = fread("/mnt/rreal/RDS/RDS/acarrasco/COLLABS/FOR_RAQUEL/DATA/GTEX_V8/sQTL_EUR_CORTEX/gtex_proccessed_CORTEX_chr2.noDUPs_COLOCformatOK.txt")
```
...
gwas_path = "/mnt/rreal/RDS/RDS/acarrasco/COLLABS/FOR_RAQUEL/DATA/Raquel_LRP1B_progToDementia/harmonised_progtoDemGWAS_V3.txt"
gtex_cortex_path <- "/mnt/rreal/RDS/RDS/acarrasco/COLLABS/FOR_RAQUEL/DATA/GTEX_V8/sQTL_EUR_CORTEX/gtex_proccessed_CORTEX_chr2.noDUPs_COLOCformatOK.txt"
...
```{r}
results_sQTL <- merge_coloc_summaries("/mnt/rreal/RDS/RDS/acarrasco/COLLABS/FOR_RAQUEL/RESULTS_sQTL_GTEX_CORTEX/progtoDementia_sQTL_GTEX_CORTEX_CHR2_LRP1B/",
...
We do not have any evidence of any LRP1B being an sQTL, leading the the progression to Dementia phenotype.
We have used GTEXV8 CORTEX Full Sum stats, having Gene-SNP pairs data across 8 introns available for the study.
